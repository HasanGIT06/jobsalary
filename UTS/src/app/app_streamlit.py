...
model_cat = joblib.load(Path(__file__).parent.parent/"artifacts/model_cat.pkl")
model_num = joblib.load(Path(__file__).parent.parent/"artifacts/model_num.pkl")
...
def make_prediction(features):
    ...
    X = preprocess_data_cat(features, is_train=False)
    Y = preprocess_data_num(features, is_train=False)
    if "placement_status" in X.columns:
        X = X.drop(columns=['placement_status'])
    if "salary_package_lpa" in Y.columns:
        Y = Y.drop(columns=['salary_package_lpa'])
    pred_cat = model_cat.predict(X)
    pred_num = model_num.predict(Y)
    return {"placement_prediction": pred_cat[0], "salary_prediction": pred_num[0]}
